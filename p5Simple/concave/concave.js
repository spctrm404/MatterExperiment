...
function setup() {
  ...
  matterObjs.push(
    new FromConcaveBody(
      Bodies.fromVertices(width * 0.5 - 150, height * 0.5, arrow, null, true)
    ).setFillColor(colors[Math.floor(random(colors.length))])
  );
  matterObjs.push(
    new FromConcaveBody(
      Bodies.fromVertices(width * 0.5 - 50, height * 0.5, chevron, null, true)
    ).setFillColor(colors[Math.floor(random(colors.length))])
  );
  matterObjs.push(
    new FromConcaveBody(
      Bodies.fromVertices(width * 0.5 + 50, height * 0.5, star, null, true)
    ).setFillColor(colors[Math.floor(random(colors.length))])
  );
  matterObjs.push(
    new FromConcaveBody(
